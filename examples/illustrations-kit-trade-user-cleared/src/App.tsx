import type { ComponentType, SVGProps } from "react";
import {
  AnvilProvider,
  Chip,
  Flex,
  Text,
  usePrefersColorScheme,
} from "@servicetitan/anvil2";
import {
  EmptyStateCommercialEmergencyComplianceUserCleared,
  EmptyStateCommercialExteriorsUserCleared,
  EmptyStateCommercialInteriorsUserCleared,
  EmptyStateCommercialMechanicalUserCleared,
  EmptyStateCommercialPropertyCareUserCleared,
  EmptyStateResidentialEvergreenUserCleared,
} from "@servicetitan/anvil2-illustrations";

// Matches the Chip fills from Figma; Chip derives the border and text
// colors from this background automatically via useAccessibleColor.
const RESIDENTIAL_CHIP_COLOR = "#e0f2ff";
const COMMERCIAL_CHIP_COLOR = "#f1edff";

type Trade = {
  label: string;
  Illustration: ComponentType<SVGProps<SVGSVGElement>>;
};

const COMMERCIAL_TRADES: Trade[] = [
  {
    label: "Property Care",
    Illustration: EmptyStateCommercialPropertyCareUserCleared,
  },
  {
    label: "Exteriors",
    Illustration: EmptyStateCommercialExteriorsUserCleared,
  },
  {
    label: "Mechanical",
    Illustration: EmptyStateCommercialMechanicalUserCleared,
  },
  {
    label: "Emergency & Compliance",
    Illustration: EmptyStateCommercialEmergencyComplianceUserCleared,
  },
  {
    label: "Interiors",
    Illustration: EmptyStateCommercialInteriorsUserCleared,
  },
];

function IllustrationBox({
  Illustration,
}: {
  Illustration: ComponentType<SVGProps<SVGSVGElement>>;
}) {
  return (
    <div
      style={{
        height: 116,
        width: 170,
        display: "flex",
        alignItems: "flex-end",
        justifyContent: "center",
      }}
    >
      <Illustration
        aria-hidden="true"
        style={{ maxWidth: 170, maxHeight: 116, width: "auto", height: "auto" }}
      />
    </div>
  );
}

function TradeLabel({ children }: { children: string }) {
  return (
    <Text
      subdued
      style={{
        fontSize: 10,
        textTransform: "uppercase",
        letterSpacing: "0.02em",
        textAlign: "center",
      }}
    >
      {children}
    </Text>
  );
}

function App() {
  const { mode } = usePrefersColorScheme();

  return (
    <AnvilProvider themeData={{ mode }}>
      <Flex direction="column" gap={6}>
        <Flex direction="column" gap={3} style={{ width: "fit-content" }}>
          <Chip
            label="Residential"
            size="small"
            color={RESIDENTIAL_CHIP_COLOR}
          />
          <Flex direction="column" alignItems="center" gap={2}>
            <IllustrationBox
              Illustration={EmptyStateResidentialEvergreenUserCleared}
            />
            <TradeLabel>
              Property Care, Exteriors, Mechanical, Emergency & Compliance,
              Interiors
            </TradeLabel>
          </Flex>
        </Flex>
        <Flex direction="column" gap={3} style={{ width: "fit-content" }}>
          <Chip label="Commercial" size="small" color={COMMERCIAL_CHIP_COLOR} />
          <Flex gap={3} alignItems="flex-start">
            {COMMERCIAL_TRADES.map(({ label, Illustration }) => (
              <Flex key={label} direction="column" alignItems="center" gap={2}>
                <IllustrationBox Illustration={Illustration} />
                <TradeLabel>{label}</TradeLabel>
              </Flex>
            ))}
          </Flex>
        </Flex>
      </Flex>
    </AnvilProvider>
  );
}

export default App;
